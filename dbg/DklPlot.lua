...
function DklBaseGraphics:donutChart(x,args)
	args = args or {}
	...
	local lastAngle = 0;
  	...
  	for i = 1, #x, 1 do
	    local gray = map(i, 0, #x, 0, 255);
	    --fill(gray);

	    arc(width()/2, height()/2, args.radius*2, args.radius*2, lastAngle, lastAngle + radians(x[i]));
	    lastAngle = lastAngle + radians(x[i]);
  	end
  	endShape()

end

function DklBaseGraphics:stepLines(x,y,args)
	args = args or {}
	...
	if (#x == #y) then
		...
		beginShape()
			...
			for i = 1, #x do
				if (i ~= #x) then -- Para validar que exista i+1
					horLineLen = x[i+1] - x[i]

					-- Horizontal line
					line(
						x[i]*self.plt.xscl, 
						-y[i]*self.plt.yscl, 
						(x[i]+horLineLen)*self.plt.xscl, 
						-y[i]*self.plt.yscl
					)
		...
		print("STEP LINE ERROR: x must be equal to y")
	end
